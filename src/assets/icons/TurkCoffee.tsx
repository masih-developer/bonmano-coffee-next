export default function TurkCoffee() {
  return (
    <svg
      id="Layer_2"
      data-name="Layer 2"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 20.01 20"
      className="size-5"
    >
      <defs>
        <style
          dangerouslySetInnerHTML={{
            __html:
              "\n      .cls-1 {\n        fill: #c19277;\n      }\n\n      .cls-1, .cls-2, .cls-3 {\n        stroke-width: 0px;\n      }\n\n      .cls-4 {\n        stroke-linecap: round;\n      }\n\n      .cls-4, .cls-2, .cls-5 {\n        fill: none;\n      }\n\n      .cls-4, .cls-5 {\n        stroke: #575756;\n        stroke-miterlimit: 10;\n      }\n\n      .cls-3 {\n        fill: #575756;\n      }\n    ",
          }}
        />
      </defs>
      <g id="Variations">
        <g id="Turkish_Coffee" data-name="Turkish Coffee">
          <line className="cls-4" x1="9.9" y1="6.52" x2="19.51" y2="4.33" />
          <path
            className="cls-1"
            d="m12.22,16.31l-2.39-11.97H2.41l-.91,4.57s-.01.03-.01.04l-.03.12v.04c0,.24.18.44.41.49.03.01.06.01.1.01.24,0,.44-.18.49-.4l.77-3.87h5.78l2.23,11.18-.76,1.24-8.74-.02-.74-1.24.47-2.34s.02-.09.02-.14c0-.24-.17-.44-.39-.49h-.02s-.06,0-.09,0c-.24,0-.44.17-.49.4L.02,16.31c-.05.25-.01.51.13.73l.73,1.22c.18.31.52.5.88.5h8.72c.36,0,.7-.19.89-.5l.73-1.22c.13-.22.17-.48.12-.73Z"
          />
          <path
            className="cls-3"
            d="m9.86,2.24l-.74,2.12H3.12l-.74-2.12h7.48m.68-1H1.7c-.35,0-.6.35-.49.69l.99,3.44h7.82l1.01-3.44c.12-.33-.13-.69-.49-.69h0Z"
          />
          <g>
            <path
              className="cls-3"
              d="m6.68,8.92c.26,0,.5.06.73.17,1.06.55,1.29,2.22.51,3.73-.61,1.19-1.66,1.93-2.6,1.93-.26,0-.5-.06-.73-.17-1.06-.55-1.29-2.22-.51-3.73.61-1.19,1.66-1.93,2.6-1.93m0-1h0c-1.35,0-2.72.97-3.49,2.47-.47.91-.66,1.92-.54,2.82.14,1.02.68,1.85,1.48,2.26.37.19.77.28,1.19.28,1.35,0,2.72-.97,3.49-2.47,1.05-2.04.64-4.27-.94-5.08-.37-.19-.77-.28-1.19-.28h0Z"
            />
            <path
              className="cls-5"
              d="m7.51,8.51c.14,1.12-.11,1.83-.37,2.29-.77,1.33-2.25,1.37-2.79,2.66-.25.61-.18,1.22-.08,1.65"
            />
          </g>
          <rect className="cls-2" x={0} width={20} height={20} />
        </g>
      </g>
    </svg>
  );
}
